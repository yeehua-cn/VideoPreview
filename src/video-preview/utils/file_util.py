...
# 定义图片文件信息结构
ImageFileInfo = namedtuple('ImageFileInfo', [
    'name', 'path', 'size', 'extension', 'modified_time'
])
...
class ImageDirectoryReader:
    ...
    IMAGE_EXTENSIONS = {
        ...
        '.dng', '.heic', '.heif', '.avif', '.jp2', '.j2k', '.pbm',
        '.pgm', '.ppm', '.pnm', '.hdr', '.exr', '.psd', '.ai', '.eps'
    }
    ...
    def _sort_files(
        ...
        Args:
            files (List[ImageFileInfo]): 文件列表
            sort_by (str): 排序字段
            reverse (bool): 是否反向排序
            
        Returns:
            List[ImageFileInfo]: 排序后的文件列表
        """
        if sort_by == "name":
...
# 使用示例和便捷函数
def get_image_files(directory: str, recursive: bool = False) -> List[ImageFileInfo]:
    """
    便捷函数：获取目录下的图片文件列表
...
def test_image_directory_reader():
    ...
    try:
        ...
        image_files = reader.get_image_files(target_dir, recursive=True, sort_by="size", reverse=True)
        ...
        if not image_files:
            ...
            for file_info in image_files[:20]:  # 只显示前20个文件
                ...
                print(f"{file_info.name:<30} {size_mb:.2f}MB {file_info.extension:<8} {mod_time}")
        ...
        # 按扩展名分组统计
        print(f"\n按扩展名分组统计:")
        grouped = reader.get_image_files_grouped_by_extension(target_dir, recursive=True)
        for ext, files in sorted(grouped.items()):
            print(f"  {ext}: {len(files)} 个文件")
        
        # 只获取特定扩展名的文件
        print(f"\n只获取JPG和PNG文件:")
        jpg_png_files = reader.get_image_files_by_extensions(target_dir, ['.jpg', '.png'], recursive=True)
...
def test_size_formatter():
    ...
    for size in test_sizes:
        ...
        print(f"{size:>20} bytes = {iec_fmt:>10} (IEC) = {si_fmt:>10} (SI) = {auto_fmt:>10} (Auto)")
