...
class LabelWidget(BoxLayout):
    def __init__(self, **kwargs):
        ...
        # 带样式的Label
        label2 = Label(
            text="Styled Text", font_size="24sp", color=(1, 0, 0, 1)  # RGBA格式颜色
        )
        label3 = Label(
            text="[b]Bold[/b] [i]Italic[/i] [color=ff0000]Red[/color]", markup=True
        )
...
class CheckBoxWidget(BoxLayout):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.orientation = "vertical"
        checkbox = CheckBox(color=(1, 1, 0.2, 1), active=True)
        checkbox.bind(active=self.on_checkbox_active)
        self.add_widget(checkbox)

        checkbox2 = CheckBox(color=(1, 0.1, 0.2, 1), active=True)
...
class TextInputWidget(BoxLayout):
    def __init__(self, **kwargs):
        ...
        self.orientation = "vertical"
        ti = TextInput(
            text="copy.com", size_hint=(0.1, 0.1)
        )  # 占父容器的80%宽度和20%高度)
        self.add_widget(ti)
        textinput = TextInput(
            multiline=False,
...
class SliderWidget(BoxLayout):
    ...
    def update_range(self, instance, value):
        self.label.text = (
            f"范围: {instance.min}-{instance.max} currnt value: {value:.2f}"
        )
...
# from kivy.core.text import LabelBase
# LabelBase.register('朱雀仿宋', 'zqfs.ttf')
